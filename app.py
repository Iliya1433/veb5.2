...
def visit_logs():
    try:
        ...
        logger.error(f"Ошибка при просмотре логов: {str(e)}")
        flash('Произошла ошибка при загрузке логов посещений')
        return redirect(url_for('index'))
...
with app.app_context():
    try:
        # Проверяем существование таблицы VisitLog
        if not db.engine.dialect.has_table(db.engine, 'visit_log'):
            logger.info("Создание таблицы VisitLog...")
            VisitLog.__table__.create(db.engine)
            logger.info("Таблица VisitLog создана успешно")
